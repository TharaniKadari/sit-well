...
// --- Voice guidance, using the browser's built-in text-to-speech ---
...
function speak(text) {
  ...
  window.speechSynthesis.cancel(); // don't stack overlapping phrases
...
function evaluatePosture(pose) {
  ...
  if (!baseline) {
    ...
    setStatus(`Getting ready... (${calibrationSamples.length}/${CALIBRATION_FRAMES})`);
    if (calibrationSamples.length >= CALIBRATION_FRAMES) {
      ...
      smoothedScore = 100;
      setStatus('Ready — watching your posture');
    }
    return;
  }
...
function updateGuidance(score, vDrift, hDrift) {
  ...
  if (score >= 75) {
    ...
    headline.textContent = vDrift > hDrift ? 'You\'re starting to slouch' : 'You\'ve leaned to the side';
    ...
    lastSpokenPhrase = vDrift > hDrift ? 'You are starting to slouch.' : 'You have leaned to the side.';
  } else {
    headline.textContent = vDrift > hDrift ? 'Sit up straight' : 'Move back to center';
    headline.style.color = 'var(--bad)';
    detail.textContent = vDrift > hDrift
      ? 'Your head has dropped well below your normal position — sit up and bring your eyes back level.'
      : 'You\'ve leaned well off to one side — move back to the middle, in front of the camera.';
    lastSpokenPhrase = vDrift > hDrift ? 'You are slouching. Please sit up straight.' : 'You have moved off to the side. Please move back to the center.';
  }
}
...
function triggerAlarm() {
  ...
  playChime();
  speak(lastSpokenPhrase);
  chimeIntervalId = setInterval(() => {
    playChime();
    speak(lastSpokenPhrase);
  }, ALARM_REPEAT_MS); // gentle nudge, not continuous
...
function stop() {
  ...
  document.getElementById('guidanceHeadline').textContent = 'Click Start to get set up';
  document.getElementById('guidanceHeadline').style.color = 'var(--ink)';
  document.getElementById('guidanceDetail').textContent = 'Sit the way you normally do. Sit Well remembers this as your normal position, and lets you know if you move away from it.';
}
