...
function evaluatePosture(pose) {
  ...
  if (!baseline) {
    baseline = { eyeMidY, eyeMidX, shoulderMidY };
  ...
  const drift = verticalDrift * 1.2 + horizontalDrift * 0.8;

  // Map drift (pixels) to a 0-100 score. Tune divisor if too sensitive/loose.
  const score = Math.round(constrain(100 - drift * 1.8, 0, 100));
  currentScore = score;
...
function triggerAlarm() {
  ...
  playChime();
  chimeIntervalId = setInterval(playChime, 3000); // gentle nudge every 3s, not continuous
